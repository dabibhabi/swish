...
void TaaPass::record(VkCommandBuffer cmd, uint32_t frameIndex, VkImage hdrImage, const TaaParams& params,
    ...
    // Resolve into taa[writeIdx].
    {
        ...
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipeLayout, 0, 1, &m_sets[writeIdx], 0, nullptr);
...
void TaaPass::createImages(const RendererServices& s) {
    for (uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        m_images[i] = gpu::deviceLocalImage(s.allocator, m_extent.width, m_extent.height, m_format,
