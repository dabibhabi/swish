...
void SceneGeometry::upload(const RendererServices& s, const MeshData& mesh, const std::vector<DrawCall>& draws) {
    ...
    uploadViaStaging(s, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, sizeof(Vertex) * mesh.getVertices().size(),
                     mesh.getVertices().data(), m_vertexBuffer, m_vertexBufferMemory);

    uploadViaStaging(s, VK_BUFFER_USAGE_INDEX_BUFFER_BIT, sizeof(uint32_t) * mesh.getIndices().size(),
